...
function displayBook(bookIndex) {
  ...
  dropDownButton.addEventListener("click", () => {
    ...
    if (compStyle === "flex") dropDownContent.style.display = "none";
    if (compStyle === "none") dropDownContent.style.display = "flex";
  });
  editButton.addEventListener("click", () => {
    ...
    if (bookObj.read) input1.checked = true;
  ...
  saveEdit.addEventListener("click", () => {
    ...
    let insertBook = new Book(
      saveBook.getElementsByTagName("input")[0].value,
      saveBook.getElementsByTagName("input")[1].value,
...
title.addEventListener("input", function (event) {
  if (title.validity.valueMissing) {
    ...
    title.setCustomValidity("Title can't be longer than 25 characters.");
    title.reportValidity();
  } else {
...
author.addEventListener("input", function (event) {
  if (author.validity.valueMissing) {
    ...
    author.setCustomValidity("Author can't be longer than 25 characters.");
    author.reportValidity();
  } else {
...
pages.addEventListener("input", function (event) {
  if (pages.validity.valueMissing) {
    pages.setCustomValidity("pages can't be empty.");
    pages.reportValidity();
  } else if (pages.validity.rangeOverflow) {
    pages.setCustomValidity("out of bound");
    pages.reportValidity();
  } else {
...
function displayAllBooks() {
  ...
  if (localStorage.length > 0) {
    ...
    if (emptyPage != null) emptyPage.style.display = "none";
    while (count < localStorage.length) {
      if (!localStorage.getItem(i)) i++;
      else if (localStorage.getItem(i)) {
        displayBook(i);
